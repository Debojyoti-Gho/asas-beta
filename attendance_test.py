...
# Inject JavaScript to handle device ID using cookies
st.markdown("""
    <script src="https://cdn.jsdelivr.net/npm/@fingerprintjs/fingerprintjs-pro@3.0.1/dist/fp.min.js"></script>
    <script>
      function getDeviceId() {
        // Check if the device ID exists in the cookies
        if (!document.cookie.includes('device_id')) {
          // Load FingerprintJS and generate a unique visitor ID
          FingerprintJS.load().then(function(fp) {
            fp.get().then(function(result) {
              const deviceId = result.visitorId;  // Unique ID generated by FingerprintJS
              // Set the device ID in cookies with an expiration time of 1 year
              document.cookie = "device_id=" + deviceId + "; expires=" + new Date(Date.now() + 365*24*60*60*1000).toUTCString() + "; path=/";
              // Reload the page with the device ID in the URL query parameter
              window.location.href = "?device_id=" + deviceId;
            });
          });
        } else {
          // If the device ID exists in cookies, redirect with the ID in URL
...
# Retrieve the device ID from the URL query parameters
