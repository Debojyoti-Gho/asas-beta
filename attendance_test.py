...
<script>
    async function registerPasskey() {
        try {
            // Securely generate a challenge (this could be done on the server side)
            const challenge = new Uint8Array(32);  // Example: Generate random challenge on server side

            // WebAuthn registration options
            const publicKey = {
                challenge: challenge,  // Challenge generated by your backend
                rp: {
                    name: "asas-beta-by-debojyotighosh.streamlit.app"  // Replace with your website name
                },
                user: {
                    id: new TextEncoder().encode('user123'),  // Replace with dynamic user ID
                    name: "asas_user",  // Replace with user's email or username
                    displayName: "ASAS USER"  // Replace with user's full name
                },
                pubKeyCredParams: [
                    { type: "public-key", alg: -7 }  // ECDSA with SHA-256
                ],
                timeout: 60000,  // 60 seconds timeout
                userVerification: "required"  // User verification required (e.g., fingerprint or PIN)
            };
            ...
            document.getElementById('registration-result').innerHTML = 'Registration failed. Please try again later.';
        }
    }
</script>
